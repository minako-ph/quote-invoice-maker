...
- 2026-07-24 【§9-5受入E2E合格】サンプル→見積PDF→請求変換→PDF→Drive確認=実測約1分（受入値3分以内）。見積Q系・請求I系の独立採番／同名衝突_2連番／無料枠3枚消費→到達メッセージverbatim表示／免税モード=登録番号行なし・税込区分記載様式を目視／F-3実機=「帳票」フォルダ削除→次回出力でフォルダ・スクラッチとも自動再作成／(dev)使用量リセットで再検証可能を確認
- 2026-07-24 §9-5残タスク: §9-6課金E2E（バックエンドデプロイ後）／源泉ON請求書の見出し金額仕様の実機目視／版指定デプロイ前の(dev)一括削除＋scratch sheetId=0前提のリトライ堅牢化＋urlFetchWhitelist縮小（www.googleapis.com はスパイク④専用のため本実装後に不要か再確認）
- 2026-07-24 【仕様確定】源泉ONの請求書は見出し「ご請求金額」=差引請求額とし、直下に補助行「（税込合計 ¥X ／ 源泉徴収税額 ▲¥Y）」を小さく表示。見積書は源泉ONでも「お見積金額」=税込合計のまま（源泉は支払時処理のため）。源泉OFFは両書類とも税込合計（現行どおり）。requirements FR-4に反映済み
- 2026-07-24 実機PDF残差修正: ①数量列は numberFormat を出さない方式へ（'#,##0.##' もSheets仕様で整数に「1.」表示＝パターン変更では解決しない実測。テストも「数量列にnumberFormat未設定」の構造検証へ差し替え） ②源泉OFF時はE列幅を12pxへ極小化（ON=60px。列幅差をテスト） ③(dev)使用量リセット devResetUsage を追加（当月カウンタ＋reviewPromptShown削除。E2Eの無料枠再検証用。**版指定デプロイ前の(dev)一括削除TODOの対象に含める**）
